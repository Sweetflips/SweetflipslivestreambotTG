...
export class TelegramBot {
    ...
    constructor(prisma, redis, rateLimiter) {
        ...
        this.commands = new TelegramCommands(prisma, bonusService, triviaService, payoutService, linkService);
        this.setupMiddlewares(authService);
        this.setupCommands();
    }
    setupMiddlewares(authService) {
    ...
    setupCommands() {
        ...
        const modMiddleware = createRBACMiddleware('MOD');
        const modRateLimit = rateLimitMiddleware(RATE_LIMITS.MOD_COMMAND);
        // Bonus Hunt Commands
        this.bot.command('start_hunt', modMiddleware, modRateLimit, ctx => this.commands.startHunt(ctx));
        this.bot.command('add_bonus', modMiddleware, modRateLimit, ctx => this.commands.addBonus(ctx));
        this.bot.command('open_bonus', modMiddleware, modRateLimit, ctx => this.commands.openBonus(ctx));
        this.bot.command('close_hunt', modMiddleware, modRateLimit, ctx => this.commands.closeHunt(ctx));
        // Trivia Commands
        this.bot.command('start_trivia', modMiddleware, modRateLimit, ctx => this.commands.startTrivia(ctx));
        this.bot.command('q', modMiddleware, modRateLimit, ctx => this.commands.postQuestion(ctx));
        this.bot.command('lock_round', modMiddleware, modRateLimit, ctx => this.commands.lockRound(ctx));
        this.bot.command('stop_trivia', modMiddleware, modRateLimit, ctx => this.commands.stopTrivia(ctx));
        // Utility Commands
        this.bot.command('state', modMiddleware, modRateLimit, ctx => this.commands.showState(ctx));
        this.bot.command('reset_game', modMiddleware, modRateLimit, ctx => this.commands.resetGame(ctx));
        this.bot.command('payout_preview', modMiddleware, modRateLimit, ctx => this.commands.payoutPreview(ctx));
        this.bot.command('link_status', modMiddleware, modRateLimit, ctx => this.commands.linkStatus(ctx));
        // Admin Management Commands (OWNER only)
        const ownerMiddleware = createRBACMiddleware('OWNER');
        ...
        const viewerMiddleware = createRBACMiddleware('VIEWER');
        const viewerRateLimit = rateLimitMiddleware(RATE_LIMITS.VIEWER_COMMAND);
        this.bot.command('link', viewerMiddleware, viewerRateLimit, ctx => this.commands.linkAccount(ctx));
        this.bot.command('unlink', viewerMiddleware, viewerRateLimit, ctx => this.commands.unlinkAccount(ctx));
        // Guess Game Commands (viewer commands)
        this.bot.command('guess', viewerMiddleware, viewerRateLimit, ctx => this.commands.guess(ctx));
        this.bot.command('gtbalance', viewerMiddleware, viewerRateLimit, ctx => this.commands.gtBalance(ctx));
        this.bot.command('gtbonus', viewerMiddleware, viewerRateLimit, ctx => this.commands.gtBonus(ctx));
        // Admin Guess Game Commands (require MOD role)
        this.bot.command('balance', modMiddleware, modRateLimit, ctx => this.commands.balanceAdmin(ctx));
