...
export class TelegramCommands {
    ...
    guessCommands;
    scheduleService;
    constructor(prisma, bonusService, triviaService, payoutService, linkService) {
        this.prisma = prisma;
    ...
    async openBonus(ctx) {
        ...
        try {
            ...
            await ctx.reply(`💰 **${bonusName}**: ${amountX}x\n\n` +
                `Use /close_hunt to finish and compute final results.`, { parse_mode: 'Markdown' });
        }
        catch (error) {
    ...
    async closeHunt(ctx) {
        try {
            ...
            result.entries.slice(0, 10).forEach((entry, index) => {
                ...
                message += `${medal} ${entry.user.kickName || entry.user.telegramUser || 'Unknown'}: ${entry.guess} (Δ${delta})\n`;
            });
    ...
    async startTrivia(ctx) {
        try {
            ...
            await ctx.reply(`🧠 **Trivia Game Started!**\n\n` +
                ...
                `Use /stop_trivia to end the game.`, { parse_mode: 'Markdown' });
        }
        catch (error) {
    ...
    async postQuestion(ctx) {
        ...
        try {
            ...
            await ctx.reply(`❓ **Question Posted!**\n\n` +
                `Question: ${question}\n` +
                `Round ID: \`${round.id}\`\n\n` +
                `Use /lock_round to stop accepting answers.`, { parse_mode: 'Markdown' });
        }
        catch (error) {
        ...
        try {
            ...
            if (result.correctAnswers.length > 0) {
                ...
                result.correctAnswers.forEach((answer, index) => {
                    message += `${index + 1}. ${answer.user.kickName || answer.user.telegramUser || 'Unknown'}\n`;
                });
            }
            await ctx.reply(message, { parse_mode: 'Markdown' });
        }
    ...
    async stopTrivia(ctx) {
        try {
            ...
            result.scores.slice(0, 10).forEach((score, index) => {
                const medal = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${index + 1}.`;
                message += `${medal} ${score.user.kickName || score.user.telegramUser || 'Unknown'}: ${score.points} points\n`;
            });
            await ctx.reply(message, { parse_mode: 'Markdown' });
        }
        catch (error) {
        ...
        try {
            ...
            if (currentGame.type === 'BONUS') {
                ...
                currentGame.scores.slice(0, 5).forEach((score, index) => {
                    message += `${index + 1}. ${score.user.kickName || score.user.telegramUser || 'Unknown'}: ${score.points}\n`;
                });
            }
            await ctx.reply(message, { parse_mode: 'Markdown' });
        }
    ...
    async payoutPreview(ctx) {
        try {
            ...
            for (const treasurerId of treasurerIds) {
                try {
                    await ctx.telegram.sendMessage(treasurerId, `💰 **Payout Instructions**\n\n` +
                        `Game: ${preview.gameId}\n` +
            ...
            await ctx.telegram.sendMessage(env.TELEGRAM_PAYOUT_GROUP_ID, `🎉 **Payout Complete!**\n\n` +
                `Game: ${preview.gameId}\n` +
                `Winners: ${preview.winners.length}\n\n` +
                `Payout instructions sent to treasurer.`, { parse_mode: 'Markdown' });
    ...
    async linkAccount(ctx) {
        try {
            ...
            await ctx.reply(`🔗 **Account Linking**\n\n` +
                ...
                `\`!link ${code}\`\n\n` +
                `This code expires in 10 minutes.`, { parse_mode: 'Markdown' });
        }
        catch (error) {
    ...
    async setRole(ctx) {
        ...
        try {
            ...
            if (args.length < 2) {
                await ctx.reply('🔧 **Set User Role**\n\n' +
                    'Usage: `/setrole <telegram_id> <MOD|OWNER>`\n\n' +
                    'Example: `/setrole 123456789 MOD`\n' +
                    'Example: `/setrole 123456789 OWNER`', { parse_mode: 'Markdown' });
                return;
            }
            ...
            if (!existingUser) {
                await ctx.reply(`❌ User with Telegram ID ${telegramId} not found.\n\n` +
                    'Make sure the user has used /start or /kick command first.', { parse_mode: 'Markdown' });
                return;
            }
            // Update user role
            ...
            await ctx.reply(`✅ **Role Updated Successfully**\n\n` +
                ...
                `Role: ${updatedUser.role}`, { parse_mode: 'Markdown' });
    ...
    async addScheduleEntry(ctx) {
        const args = ctx.message && 'text' in ctx.message ? ctx.message.text.split(' ').slice(1) : [];
        if (args.length < 3) {
            await ctx.reply('❌ Usage: /schedule add <day> <stream> <title>\n\n' +
        ...
        try {
            ...
            await ctx.reply(`✅ **Schedule Entry Added**\n\n` +
                `Day: ${days[dayOfWeek]}\n` +
                `Time: ${timeStr}\n` +
                `Title: ${eventTitle}`, { parse_mode: 'Markdown' });
    ...
    async removeScheduleEntry(ctx) {
        const args = ctx.message && 'text' in ctx.message ? ctx.message.text.split(' ').slice(1) : [];
        if (args.length < 2) {
            await ctx.reply('❌ Usage: /schedule remove <day> <stream>\n\n' +
                'Days: 0=Sunday, 1=Monday, 2=Tuesday, 3=Wednesday, 4=Thursday, 5=Friday, 6=Saturday\n' +
                'Streams: 1=8am UTC, 2=6pm UTC\n\n' +
