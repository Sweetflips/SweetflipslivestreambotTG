...
export class GuessCommands {
    ...
    guessService;
    bonusService;
    constructor(prisma, guessService, bonusService) {
        this.prisma = prisma;
    ...
    async handleGtBalance(ctx, args) {
        try {
            ...
            if (!(await this.isUserLinked(telegramId))) {
                await ctx.reply('🔗 **Account Linking Required**\n\n' +
                    'You must link your Kick account before participating in games.\n\n' +
                    'Use /kick to link your account.', { parse_mode: 'Markdown' });
                return;
            }
            const user = await this.getUser(telegramId);
            // If no number provided, prompt for input
            if (args.length === 0) {
                await ctx.reply('🎯 **Guess the Balance**\n\n' +
                    'Send your balance guess now (one time).\n\n' +
                    'Example: `/gtbalance 50000`', { parse_mode: 'Markdown' });
                return;
            }
    ...
    async handleGuess(ctx, args) {
        try {
            ...
            if (args.length < 2) {
                await ctx.reply('🎯 **Guess Commands**\n\n' +
                    'Usage:\n' +
            ...
            const result = await this.guessService.submitGuess(user.id, gameType === 'balance' ? GameType.GUESS_BALANCE : GameType.GUESS_BONUS, guessValue);
            await ctx.reply(result.message, { parse_mode: 'Markdown' });
        }
        catch (error) {
    ...
    async handleGtBonus(ctx, args) {
        try {
            ...
            // If no number provided, prompt for input
            if (args.length === 0) {
                await ctx.reply('🎯 **Guess the Bonus Total**\n\n' +
                    'Send your bonus total guess now (one time).\n\n' +
                    'Example: `/gtbonus 150`', { parse_mode: 'Markdown' });
                return;
            }
            // Parse and validate number
            const guess = parseInt(args[0]);
    ...
    async handleBalanceAdmin(ctx, args) {
        try {
            const telegramId = ctx.from?.id?.toString();
            if (!telegramId) {
                await ctx.reply('❌ Unable to identify user.');
            ...
            const user = await this.getUser(telegramId);
            if (args.length === 0) {
                await ctx.reply('🔧 **Balance Admin Commands**\n\n' +
                    'Available commands:\n' +
            ...
            switch (command) {
                ...
                case 'final':
                    ...
                    const finalResult = await this.guessService.setFinalValue(GameType.GUESS_BALANCE, finalValue, user.id);
                    await ctx.reply(finalResult, { parse_mode: 'Markdown' });
                    break;
                case 'reveal':
                    const topN = this.parseTopN(args);
                    const revealResult = await this.guessService.revealResults(GameType.GUESS_BALANCE, user.id, topN);
                    await ctx.reply(revealResult, { parse_mode: 'Markdown' });
                    break;
                case 'show':
                    const showTopN = this.parseTopN(args);
                    const showResult = await this.guessService.showStandings(GameType.GUESS_BALANCE, showTopN);
                ...
                case 'reset':
                    ...
                    if (args.length < 2 || args[1] !== 'CONFIRM') {
                        await ctx.reply('⚠️ **Reset Confirmation Required**\n\nThis will clear all guesses and results.\n\nUse `/balance reset CONFIRM` to proceed.', { parse_mode: 'Markdown' });
                        return;
                    }
                ...
                case 'grace':
                    ...
                    const graceResult = await this.guessService.updateConfig(GameType.GUESS_BALANCE, { graceWindow: graceSeconds }, user.id);
                ...
                case 'window':
                    ...
                    const windowResult = await this.guessService.updateConfig(GameType.GUESS_BALANCE, { windowMin: windowMinutes }, user.id);
                ...
                case 'range':
                    ...
                    const rangeResult = await this.guessService.updateConfig(GameType.GUESS_BALANCE, { minRange, maxRange }, user.id);
    ...
    async handleBonusAdmin(ctx, args) {
        try {
            const telegramId = ctx.from?.id?.toString();
            if (!telegramId) {
                await ctx.reply('❌ Unable to identify user.');
            ...
            const user = await this.getUser(telegramId);
            if (args.length === 0) {
                await ctx.reply('🔧 **Bonus Admin Commands**\n\n' +
                    'Available commands:\n' +
            ...
            switch (command) {
                ...
                case 'final':
                    ...
                    const finalResult = await this.guessService.setFinalValue(GameType.GUESS_BONUS, finalValue, user.id);
                    await ctx.reply(finalResult, { parse_mode: 'Markdown' });
                    break;
                case 'reveal':
                    const topN = this.parseTopN(args);
                    const revealResult = await this.guessService.revealResults(GameType.GUESS_BONUS, user.id, topN);
                    await ctx.reply(revealResult, { parse_mode: 'Markdown' });
                    break;
                case 'show':
                    const showTopN = this.parseTopN(args);
                ...
                case 'reset':
                    ...
                    if (args.length < 2 || args[1] !== 'CONFIRM') {
                        await ctx.reply('⚠️ **Reset Confirmation Required**\n\nThis will clear all guesses and results.\n\nUse `/bonus reset CONFIRM` to proceed.', { parse_mode: 'Markdown' });
                        return;
                    }
                ...
                case 'grace':
                    ...
                    const graceResult = await this.guessService.updateConfig(GameType.GUESS_BONUS, { graceWindow: graceSeconds }, user.id);
                    await ctx.reply(graceResult, { parse_mode: 'Markdown' });
                    break;
                case 'window':
                    if (args.length < 2) {
                    ...
                    const windowResult = await this.guessService.updateConfig(GameType.GUESS_BONUS, { windowMin: windowMinutes }, user.id);
                    await ctx.reply(windowResult, { parse_mode: 'Markdown' });
                    break;
                case 'range':
                    if (!(await this.guessService.isOwner(telegramId))) {
                    ...
                    const rangeResult = await this.guessService.updateConfig(GameType.GUESS_BONUS, { minRange, maxRange }, user.id);
                    await ctx.reply(rangeResult, { parse_mode: 'Markdown' });
                    break;
                // Bonus item management
                case 'add':
                ...
                case 'remove':
                    ...
                    const removeResult = await this.bonusService.removeBonusItem(args.slice(1).join(' '), user.id);
    ...
    async handleGameAdmin(ctx, args) {
        try {
            const telegramId = ctx.from?.id?.toString();
            if (!telegramId) {
                await ctx.reply('❌ Unable to identify user.');
            ...
            if (args.length < 2) {
                await ctx.reply('🔧 **Game Admin Commands**\n\n' +
                    '**Basic Commands:**\n' +
            ...
            switch (command) {
                ...
                case 'reset':
                    ...
                    if (remainingArgs.length === 0 || remainingArgs[0] !== 'CONFIRM') {
                        await ctx.reply(`⚠️ **Reset Confirmation Required**\n\nThis will clear all guesses and results (data will be archived).\n\nUse \`/game reset ${gameType} CONFIRM\` to proceed.`, { parse_mode: 'Markdown' });
                        return;
                    }
                ...
                case 'export':
                    ...
                    await ctx.reply(`📊 **${gameName} Guesses Export:**\n\n\`\`\`csv\n${exportResult}\n\`\`\``, { parse_mode: 'Markdown' });
                    break;
                case 'archive':
                    if (!(await this.guessService.isOwner(telegramId))) {
                        await ctx.reply('⛔️ Owner only.');
    ...
    async logAuditAttempt(telegramId, command, args) {
        try {
...
//# sourceMappingURL=guessCommands.js.map