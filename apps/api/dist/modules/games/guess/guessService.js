...
export var GameType;
(function (GameType) {
    GameType["GUESS_BALANCE"] = "GUESS_BALANCE";
    GameType["GUESS_BONUS"] = "GUESS_BONUS";
})(GameType || (GameType = {}));
export class GuessService {
    ...
    // Submit a guess
    async submitGuess(userId, gameType, value, isEdit = false) {
        ...
        if (round.phase !== 'OPEN') {
            if (round.phase === 'CLOSED' || round.phase === 'REVEALED') {
                return {
                    success: false,
                    message: `⛔️ Guessing is closed. ${round.phase === 'REVEALED'
        ...
        return {
            success: true,
            message: `✅ Saved *${value}*. You can edit once within ${round.graceWindow}s with /${gameType === GameType.GUESS_BALANCE ? 'gtbalance' : 'gtbonus'} again.`,
            isEdit: false,
    ...
    async openRound(gameType, userId) {
        const round = await this.getCurrentRound(gameType);
        if (round.phase === 'OPEN') {
            return `✅ *${gameType === GameType.GUESS_BALANCE ? 'Balance' : 'Bonus'}* guesses are already OPEN.`;
        }
        ...
        return `✅ *${gameType === GameType.GUESS_BALANCE ? 'Balance' : 'Bonus'}* guesses are OPEN.${windowText} One guess per user.`;
    }
    // Close a game round
    async closeRound(gameType, userId) {
        const round = await this.getCurrentRound(gameType);
        if (round.phase !== 'OPEN') {
            return `⛔️ *${gameType === GameType.GUESS_BALANCE ? 'Balance' : 'Bonus'}* guesses are not currently open.`;
        }
        ...
        return `⛔️ *${gameType === GameType.GUESS_BALANCE ? 'Balance' : 'Bonus'}* guesses are CLOSED. ${guessCount} entries collected.`;
    }
    // Set final value
    async setFinalValue(gameType, value, userId) {
        const round = await this.getCurrentRound(gameType);
        await this.prisma.gameRound.update({
            where: { id: round.id },
            data: {
        ...
        return `✅ Final ${gameType === GameType.GUESS_BALANCE ? 'balance' : 'bonus total'} set to *${value}*.`;
    }
    // Reveal results
    async revealResults(gameType, userId, topN = 10) {
        const round = await this.getCurrentRound(gameType);
        if (!round.finalValue) {
            return `❌ Final value must be set before revealing results. Use /${gameType === GameType.GUESS_BALANCE ? 'balance' : 'bonus'} final <number> first.`;
        }
        ...
        const header = `🏁 Final ${gameName}: *${round.finalValue}${gameType === GameType.GUESS_BONUS ? 'x' : ''}*`;
        let body = '';
    ...
    async completeGameRound(gameType, userId) {
        const round = await this.getCurrentRound(gameType);
        if (round.phase !== 'REVEALED') {
            return `❌ Game must be revealed before completion. Use /${gameType === GameType.GUESS_BALANCE ? 'balance' : 'bonus'} reveal first.`;
        }
    ...
    // Get leaderboard
    async getLeaderboard(gameType, topN = 10) {
        const round = await this.getCurrentRound(gameType);
        if (!round.finalValue) {
    ...
    async showStandings(gameType, topN = 10) {
        ...
        let message = `📊 **Current Standings:**\n\n${guessCount} guess${guessCount === 1 ? '' : 'es'} submitted.`;
    ...
    async resetRound(gameType, userId) {
        ...
        if (round.phase === 'COMPLETED') {
            return `❌ Cannot reset a completed game. Use /${gameType === GameType.GUESS_BALANCE ? 'balance' : 'bonus'} new to start a fresh game.`;
        }
        ...
        return `✅ ${gameType === GameType.GUESS_BALANCE ? 'Balance' : 'Bonus'} game reset. All guesses and results cleared.${archiveNote}`;
    }
    ...
    async startNewRound(gameType, userId) {
        ...
        if (currentRound.phase !== 'IDLE') {
            ...
            if (guessCount > 0) {
                return `❌ Cannot start new game while current game has ${guessCount} guess${guessCount === 1 ? '' : 'es'}. Complete or reset the current game first.`;
            }
        }
        // Create new round
        const config = this.getDefaultConfig(gameType);
        ...
        return `✅ New ${gameType === GameType.GUESS_BALANCE ? 'Balance' : 'Bonus'} game round created. Ready to open for guesses.`;
    }
    // Update game configuration
    async updateConfig(gameType, config, userId) {
        const round = await this.getCurrentRound(gameType);
        await this.prisma.gameRound.update({
            where: { id: round.id },
            data: {
        ...
        const changes = Object.entries(config)
            ...
            .join(', ');
        return `✅ ${gameType === GameType.GUESS_BALANCE ? 'Balance' : 'Bonus'} game config updated: ${changes}`;
    }
    // Export guesses as CSV data
    async exportGuesses(gameType) {
        const round = await this.getCurrentRound(gameType);
        const guesses = await this.prisma.guess.findMany({
            where: { gameRoundId: round.id },
            include: {
