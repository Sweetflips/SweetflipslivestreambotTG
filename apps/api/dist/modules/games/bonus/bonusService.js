...
export class BonusService {
    ...
    async revealGame(gameRoundId, finalValue) {
        const gameRound = await this.prisma.gameRound.findUnique({
            where: { id: gameRoundId },
        });
        if (!gameRound) {
    ...
    async submitGuess(gameRoundId, userId, guess) {
    ...
    async getGameResults(gameRoundId) {
        const gameRound = await this.prisma.gameRound.findUnique({
            where: { id: gameRoundId },
            include: {
                guesses: {
    ...
    async getLeaderboard(gameRoundId) {
        const gameRound = await this.prisma.gameRound.findUnique({
            where: { id: gameRoundId },
            include: {
                guesses: {
