...
export class Server {
  ...
  private setupRoutes() {
    // Health check
    this.fastify.get('/healthz', async (request: any, reply: any) => {
      return { status: 'ok', timestamp: new Date().toISOString() };
    });

    // Game state endpoint
    this.fastify.get('/state', async (request: any, reply: any) => {
      const state = await this.overlayService.getCurrentState();
      return state;
    });

    // Overlay state endpoint
    this.fastify.get('/overlay/state', async (request: any, reply: any) => {
      const state = await this.overlayService.getCurrentState();
      return state;
    });
    ...
    // Telegram webhook
    this.fastify.post('/webhook/telegram', async (request: any, reply: any) => {
  ...
  private setupErrorHandling() {
    this.fastify.setErrorHandler(async (error: any, request: any, reply: any) => {
