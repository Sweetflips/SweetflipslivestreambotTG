...
export class CallSessionService {
  ...
  private activeSessions: Map<string, CallSessionData>;

  constructor(prisma: PrismaClient) {
    this.prisma = prisma;
    this.activeSessions = new Map<string, CallSessionData>();
  }
  ...
  async createNewCallSession(): Promise<CallSessionData | null> {
    try {
  ...
  async getActiveCallSession(): Promise<CallSessionData | null> {
    try {
  ...
  async getSessionCallEntries(sessionId?: string): Promise<CallEntryData[]> {
    try {
  ...
  async closeCallSession(sessionId?: string): Promise<boolean> {
    try {
      let targetSessionId = sessionId;
  ...
  async revealCallSession(sessionId?: string): Promise<boolean> {
    try {
      let targetSessionId = sessionId;
