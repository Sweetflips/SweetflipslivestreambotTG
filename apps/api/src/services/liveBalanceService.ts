...
export class LiveBalanceService {
   ...
   */
  async fetchCurrentBalance(): Promise<LiveBalanceData> {
    try {
  ...
  private extractBalanceFromResponse(data: any): number {
    ...
    if (data.data && typeof data.data.balance === 'number') return data.data.balance;
    if (data.result && typeof data.result.balance === 'number') return data.result.balance;
   ...
   */
  getCachedBalance(maxAgeMinutes: number = 5): LiveBalanceData | null {
   ...
   */
  formatLeaderboard(entries: LeaderboardEntry[], showTop: number = 10): string {
    if (entries.length === 0) {
