...
export async function registerRoutes(fastify: FastifyInstance) {
  ...
  fastify.register(
    async function (fastify) {
      ...
      fastify.get(
        '/active-codes/:telegramId',
        linkController.getActiveLinkCodes.bind(linkController)
      );
  ...
  fastify.register(
    async function (fastify) {
      ...
      fastify.get('/connections', async (request, reply) => {
        try {
          const overlayService = (fastify as any).overlayService;
          if (!overlayService) {
  ...
  fastify.register(
    async function (fastify) {
      ...
      fastify.post(
        '/add',
        { preHandler: modPreHandler },
        scheduleController.addScheduleEntry.bind(scheduleController)
      );
      fastify.post(
        '/remove',
        { preHandler: modPreHandler },
        scheduleController.removeScheduleEntry.bind(scheduleController)
      );
    },
    { prefix: '/api/schedule' }
  );
  ...
  fastify.register(
    async function (fastify) {
      ...
      fastify.post('/telegram/webhook', async (request, reply) => {
        try {
          const telegramBot = (fastify as any).telegramBot;
          if (!telegramBot) {
            return reply.status(503).send({
              success: false,
