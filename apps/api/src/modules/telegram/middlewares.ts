...
export const authMiddleware: MiddlewareFn<TelegramContext> = async (ctx, next) => {
  ...
  try {
    const authResult = await ctx.authService.authenticateTelegramUser(
      telegramId,
      chatId,
...
export const rateLimitMiddleware = (config: typeof RATE_LIMITS[keyof typeof RATE_LIMITS]): MiddlewareFn<TelegramContext> => {
...
export const idempotencyMiddleware: MiddlewareFn<TelegramContext> = async (ctx, next) => {
...
export const commandValidationMiddleware: MiddlewareFn<TelegramContext> = async (ctx, next) => {
