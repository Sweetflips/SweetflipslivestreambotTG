...
export class KickChatProvider {
  ...
  private async processMessage(kickMessage: KickMessage) {
    try {
  ...
  private async handleGuess(userId: string, guess: number) {
    try {
      // Rate limiting
  ...
  private async handleLink(userId: string, code: string) {
    try {
      // Rate limiting
  ...
  private async handleAnswer(userId: string, answer: string) {
    try {
      // Rate limiting
