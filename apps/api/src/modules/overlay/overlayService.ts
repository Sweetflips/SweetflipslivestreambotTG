...
export class OverlayService {
  ...
  private setupOverlayNamespace() {
    ...
    this.overlayNamespace.on('connection', (socket: any) => {
  ...
  async getCurrentState(): Promise<OverlayState> {
    try {
  ...
  async broadcastBonusUpdate(gameId: string) {
    try {
  ...
  async broadcastBonusFinal(gameId: string, results: any) {
    try {
      this.overlayNamespace.emit('bonus.final', {
        gameId,
        totalPayout: results.totalPayout,
        winners: results.entries.slice(0, 10).map((entry: any, index: number) => ({
          position: index + 1,
          username: entry.user.kickName || entry.user.telegramUser || 'Unknown',
          guess: entry.guess,
  ...
  async broadcastTriviaUpdate(gameId: string) {
    try {
  ...
  async broadcastTriviaScores(gameId: string, scores: any[]) {
    try {
