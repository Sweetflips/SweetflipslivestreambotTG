...
export class LiveLeaderboardCommands {
  ...
  private liveBalanceService: LiveBalanceService;
   ...
   */
  async showLiveLeaderboard(ctx: Context, showTop: number = 10): Promise<void> {
    try {
   ...
   */
  async showLiveBalance(ctx: Context): Promise<void> {
    try {
      const userId = ctx.from?.id?.toString();
      if (!userId) {
        await ctx.reply('❌ Unable to identify user.');
   ...
   */
  private async getCurrentGameGuesses(): Promise<
    Array<{
  ...
  private formatLiveLeaderboard(
    ...
    topEntries.forEach(entry => {
      ...
      const difference =
        entry.difference === 0 ? 'EXACT!' : `(${entry.difference.toLocaleString()} off)`;
