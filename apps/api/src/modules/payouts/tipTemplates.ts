...
export class TipTemplateGenerator {
  static generatePayoutPreview(awards: (Award & { user: User })[]): PayoutPreview {
    const winners: PayoutWinner[] = awards.map((award, index) => ({
      userId: award.userId,
  ...
  private static calculateTotalAmount(awards: Award[]): number {
