...
async function fixSweetCallsRoundSchema() {
  try {
    console.log('🔧 Fixing SweetCallsRound schema...');
    ...
    if (!tableExists[0].exists) {
      ...
      console.log('🔄 Creating table with Prisma db push...');
      
      // Use Prisma to create the table
    ...
    try {
      const testRound = await prisma.sweetCallsRound.findFirst();
      ...
      console.log('✅ Can create new SweetCallsRound');
    ...
    console.log('🎉 SweetCallsRound schema fix completed successfully!');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  fixSweetCallsRoundSchema().catch((error) => {
    console.error('❌ Script failed:', error);
    process.exit(1);
  });
}

export { fixSweetCallsRoundSchema };
