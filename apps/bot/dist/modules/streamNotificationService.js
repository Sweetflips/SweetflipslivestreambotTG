...
export const checkForUpcomingStreams = async (prisma, hoursBefore = 2) => {
    if (!prisma) {
        return [];
    }
    const now = new Date();
...
export const hasNotificationBeenSent = async (prisma, dayOfWeek, streamNumber, notificationType, eventDate) => {
    if (!prisma) {
        return false;
    }
    try {
...
export const recordNotificationSent = async (prisma, dayOfWeek, streamNumber, notificationType, eventDate, successCount, failedCount) => {
    if (!prisma) {
        return;
    }
    try {
...
export const createStreamReminderMessage = (reminder) => {
    const times = formatStreamTimes(reminder.streamNumber === 1 ? "07:00" : "17:00", reminder.streamNumber);
...
export const cleanupOldNotifications = async (prisma) => {
    if (!prisma) {
        return;
    }
    try {
