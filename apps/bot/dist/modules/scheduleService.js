...
export const addScheduleEntry = async (prisma, entry, createdBy) => {
    if (!prisma) {
        return false;
    }
...
export const removeScheduleEntry = async (prisma, dayOfWeek, streamNumber) => {
    if (!prisma) {
        return false;
    }
...
export const getActiveSchedule = async (prisma) => {
    if (!prisma) {
        return [];
    }
    ...
    return entries.map((entry) => {
        const times = formatStreamTimes(entry.streamNumber === 1 ? "07:00" : "17:00", entry.streamNumber)
...
export const getScheduleWithCurrentDayFirst = async (prisma) => {
    if (!prisma) {
...
export const cleanupOldEvents = async (prisma) => {
    if (!prisma) {
        return;
    }
    const now = new Date();
