...
const createEnvCredentialClient = async () => {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : "Unknown environment auth error";
...
const createFileCredentialClient = async () => {
    try {
        ...
        const message = error instanceof Error ? error.message : "Unknown file auth error";
        console.error(`File-based Google auth failed: ${message}`);
        return null;
    }
};
export const createGoogleSheetsService = async () => {
    if (!env.GOOGLE_SPREADSHEET_ID) {
        return null;
    }
    const client = (await createEnvCredentialClient()) ??
        (await createFileCredentialClient());
    if (!client) {
        return null;
    }
    const appendUser = async (entries) => {
