const isValidKickName = (value) => /^[a-zA-Z0-9_]{3,20}$/.test(value);
export const handleKickLinking = async (ctx, next) => {
    if (!ctx.from) {
        await next();
        return;
    }
    ...
    await ctx.reply(`Account linked successfully. Telegram: @${updated.telegramUser ?? "unknown"}, Kick: @${kickName}`);
};
